...
def get_user_preference():
    ...
    while True:
        try:
            user_year = int(input())
            if 1900 <= user_year <= 2030:
                break
            else:
                print("年份需在1900-2030之间，请重新输入!")
        except ValueError:
            print("请输入整数年份，不要输入文字!")
    return user_type, min_rating,user_year

# 根据用户偏好筛选电影
def recommend_movies(user_type, min_rating, user_year, movie_db):
    """
    ...
    for movie in movie_db:
        if movie["type"] == user_type and movie["rating"] >= min_rating and movie["year"] == user_year:
...
if __name__ == "__main__":
    ...
    # 请在此处编写代码
    # ====================Begin====================
    prefer_type, min_score, prefer_year= get_user_preference()
    result_list = recommend_movies(prefer_type, min_score, prefer_year, movie_database)
